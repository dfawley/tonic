        ...
        let output = async_stream::try_stream! {
            while let Some(req) = stream.next().await {
                yield Box::new(MyResMessage(format!(
                    ...
                    id, method, (req as Box<dyn Any>).downcast_ref::<MyReqMessage>(),
                ))) as Box<dyn Message>;
            }
        };

        Response::new(Box::new(output))
    }
}
...
async fn main() {
    ...
    let outbound = async_stream::stream! {
        yield Ok(Box::new(MyReqMessage("My Request 1".to_string())) as Box<dyn Message>);
        yield Ok(Box::new(MyReqMessage("My Request 2".to_string())));
        yield Ok(Box::new(MyReqMessage("My Request 3".to_string())));
    };

    let req = Request::new(Box::new(outbound));
