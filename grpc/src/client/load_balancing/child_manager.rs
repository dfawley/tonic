...
impl<T: ChildIdentifier> LbPolicy for ChildManager<T> {
    fn resolver_update(
        ...
        for (new_idx, (identifier, builder)) in ids_builders.into_iter().enumerate() {
            ...
            {
                // Update pending work to point to new index instead of old index.
                if pending_work.contains(&old_idx) {
                    pending_work.remove(&old_idx);
                ...
                let pending_work = self.pending_work.clone();
                *work_scheduler.idx.lock().unwrap() = new_idx;
...
impl WorkScheduler for ChildWorkScheduler {
    fn schedule_work(&self) {
        let mut pending_work = self.pending_work.lock().unwrap();
        pending_work.insert(*self.idx.lock().unwrap());
    }
}
