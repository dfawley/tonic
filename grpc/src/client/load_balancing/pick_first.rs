...
struct PickFirstPolicy {
    ...
    last_resolver_error: Option<ResolutionError>, // Most recent error from the name resolver.
...
impl PickFirstPolicy {
    ...
    fn handle_empty_endpoints(&mut self, channel_controller: &mut dyn ChannelController) {
        ...
        self.addresses = vec![];
        let res_err = ResolutionError::new("received empty address list from the name resolver");
