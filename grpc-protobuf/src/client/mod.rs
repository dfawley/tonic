...
pub trait CallBuilder<C: InvokeOnce>: private::Sealed + Sized {
    #[doc(hidden)]
    type Builder<NewC: InvokeOnce>: CallBuilder<NewC>;

    #[doc(hidden)]
    fn rebuild<NewC: InvokeOnce>(self, f: impl FnOnce(C) -> NewC) -> Self::Builder<NewC>;
    ...
    fn with_interceptor<I: Intercept<C>>(
        ...
        self.rebuild(|c| c.with_interceptor(interceptor))
    }
    ...
    fn with_once_interceptor<I: InterceptOnce<C>>(
        ...
        self.rebuild(|c| c.with_once_interceptor(interceptor))
    }
}
